...
if (socialLinksContainer) {
    socialLinksContainer.innerHTML = socialLinks.map(link => `
        <a href="${link.href}" target="_blank" rel="noopener" class="p-2 bg-white dark:bg-gray-800 rounded-full shadow-md hover:shadow-lg transition-all duration-300 border dark:border-gray-700">
            ${link.svg}
        </a>
    `).join('');
}
...
if (bestSellerList) {
    bestSellerList.innerHTML = bestSellers.map(item => `
        <a href="${item.href}" class="block bg-white dark:bg-gray-800 p-4 rounded-lg shadow-md hover:shadow-lg transition-all duration-300 border dark:border-gray-700">
...
if (cardProjects) {
    ...
    cardProjects.innerHTML = projects.map(project => `
        <a href="${project.href}" class="block bg-white dark:bg-gray-800 p-4 rounded-lg shadow-md hover:shadow-lg transition-all duration-300 border dark:border-gray-700">
